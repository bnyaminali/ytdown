    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'دەنگ':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "دەنگ", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("دۆکیومێنت",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "ڤیدیۆ", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("دۆکیومێنت",
...
async def catch_youtube_dldata(c, q):
    ...
    yturl = cb_data.split("||")[-1]
    format_id = cb_data.split("||")[-2]
    if not cb_data.startswith(("ڤیدیۆ", "دەنگ", "دەنگی دۆکیومێنت", "ڤیدیۆ دۆکیۆمێنت")):
        print("داتا نەدۆزراوە")
        raise ContinuePropagation

    filext = "%(title)s.%(ext)s"
    userdir = os.path.join(os.getcwd(), "دابەزێنراوەکان", str(q.message.chat.id))

    if not os.path.isdir(userdir):
        os.makedirs(userdir)
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("دادەبەزنرێت...", callback_data="دادەبەزێ")]]))
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("بڵاودەبێتەوە...", callback_data="دادەبەزنرێت")]]))
        await c.send_chat_action(chat_id=q.message.chat.id, action="بڵاوکردنی دۆکیومێنت")
